...
**Derived config**:
A clean process-compose config that devtrees generates from `devtrees.yaml` + base config: tier keys stripped, services partitioned by tier, isolation values injected. Disposable build artifact, one per instance (shared-subset, isolated-subset). Emitted to disk on demand via `devtrees generate`, or previewed in-memory (no disk write) via `devtrees up --dry-run` — which runs the full derivation pipeline and prints the config(s) + allocated env to stdout with no side effects (no spawn, no socket, no instance registered, no health wait). The one exception is port allocation, which may briefly take the registry lock to reserve a non-colliding block, exactly as `generate` does; this reserves a block but starts and registers nothing, so a concurrent real `up` could still pick a different block.
