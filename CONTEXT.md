...
**Derived config**:
A clean process-compose config that devtrees generates from `devtrees.yaml` + base config: tier keys stripped, services partitioned by tier, isolation values injected. A `depends_on` edge between two services in the same tier is re-emitted with the `condition` its author wrote (`process_healthy`, …) — devtrees never rewrites or validates the condition; an edge authored without one gets process-compose's own default, `process_started`. Cross-tier edges are dropped and resolved at the orchestration layer instead (ADR-0003). Disposable build artifact, one per instance (shared-subset, isolated-subset). Written to disk inline by `devtrees up` as it brings the stack up, or previewed in-memory (no disk write) via `devtrees up --dry-run` — which runs the full derivation pipeline and prints the config(s) + allocated env to stdout with no side effects (no spawn, no socket, no instance registered, no health wait). The one exception is port allocation, which may briefly take the registry lock to reserve a non-colliding block; this reserves a block but starts and registers nothing, so a concurrent real `up` could still pick a different block.
...
**Control socket**:
The unix domain socket a process-compose instance exposes its API on, at a path derived from the instance's id under the **run dir** (`<run-dir>/<worktree-id>.sock`, plus a fixed `shared.sock`). Clients and devtrees talk to an instance through it; enumerating the run dir is how devtrees discovers every running instance and detects stale ones.

**Run dir**:
The single short directory holding one repo's control sockets: `<runtime-base>/devtrees-<uid>/<anchor-hash>/`, where `<runtime-base>` is `$DEVTREES_RUNTIME_DIR`, else `$XDG_RUNTIME_DIR`, else `/tmp`. It is the one piece of devtrees state that does **not** live in anchor state, because a unix socket path must fit in `sun_path` (104 bytes on macOS) and an anchor path on a deep checkout does not (issue #156, ADR-0007). It is keyed by a hash of the anchor, so it is still one namespace per repo; the socket filenames inside it are still worktree ids. Being outside the git dir, it is cleared on reboot — which is correct: a control socket is runtime state, meaningless once the process that bound it is gone.

**Anchor state**:
A `devtrees/` directory *inside* the anchor (the git common dir) — i.e. `<git-common-dir>/devtrees/` — where devtrees keeps everything it needs to reason about the whole repo's durable running state: the allocation registry, the derived configs, the per-instance logs, and the shared instance's persisted name→port map (what the running shared instance actually bound, plus a hash of the shared subset it was started from — worktrees inject shared connection info from this map rather than recomputing it, and a worktree whose shared subset diverges from it fails with `SHARED_DRIFT`). Because it lives inside the git dir it is never part of the working tree, so it needs no `.gitignore` entry and works identically for normal and bare repos.
