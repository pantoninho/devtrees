...
- All devtrees runtime state (registry, control sockets, derived configs) lives in `<git-common-dir>/devtrees/`. Because that path is inside the git dir, it is never part of the working tree — no `.gitignore` entry is needed and the same path works for normal and bare repos.
  - **Amended by [ADR-0007](0007-control-sockets-in-a-short-runtime-dir.md):** control sockets are the one exception. A unix socket path must fit in `sun_path` (104 bytes on macOS) and an anchor path on a deep checkout does not, so sockets moved to a short per-user runtime dir keyed by a hash of the anchor (issue #156). The registry, derived configs, logs and shared-state map are unaffected — the anchor is still where devtrees reasons about the repo from.
