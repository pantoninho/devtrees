...
describe("runUpDryRun — derive the config(s) with no side effects (#124)", () => {
  ...
  it("allocation may take the registry lock briefly to pick a non-colliding block (documented)", async () => {
    ...
    // does not start or register an instance).
    const stack: ResolvedStack = {
      services: [isolated("web", "node x.js", ["WEB_PORT"])],
    };
...
describe("runPrune — reconcile instances against git worktree list", () => {
  ...
  it("reports an orphan discovered via both socket and a dead registry key exactly once (issue #142)", async () => {
    // A worktree removed while still running leaves a live socket AND a
    // registry entry. The socket-keyed sweep and the registry reconciliation
    // must converge: the orphan is stopped, cleaned, and reported once.
    const tmp = tmpAnchor();
    mkdirSync(join(tmp.anchor, "devtrees", "run"), { recursive: true });
    const removedId = idFor(tmp.worktreeRoot, "feature");
    const liveId = idFor(tmp.worktreeRoot, "main");
    const orphanSocket = instancePaths(tmp.anchor, removedId).socketPath;
    const orphanConfig = instancePaths(tmp.anchor, removedId).configPath;
    writeFileSync(orphanSocket, "");
    writeFileSync(orphanConfig, "processes: {}\n");
    ...
    const registryRef = {
      snapshot: { [liveId]: 22048, [removedId]: 42560 } as RegistrySnapshot,
    };
    ...
    const result = await runPrune({
      cwd: join(tmp.worktreeRoot, "main"),
      git,
      discover: async () => [instance(removedId, { status: "running", socketPath: orphanSocket })],
      readRegistry: () => registryRef.snapshot,
      withRegistryLock: async (_anchor, mutate) => {
        const after = await mutate(registryRef.snapshot);
        registryRef.snapshot = after;
      ...
      driver: {
        exists: () => Promise.resolve(true),
        spawner: (_binary, args) => {
          if (args[0] === "down") downCalls.push(args[args.indexOf("-u") + 1] ?? "");
          return spawnedOk();
        },
      },
    });

    // Acceptance: reported exactly once, not duplicated by the registry pass.
    expect(result.pruned.map((p) => p.id)).toEqual([removedId]);
    // Acceptance: it was stopped (running → driver.down), proving it came
    // through the socket path, not the synthetic stale registry record.
    expect(downCalls).toEqual([orphanSocket]);
    expect(removedId in registryRef.snapshot).toBe(false);
    expect(existsSync(orphanConfig)).toBe(false);
  });

  it("preserves __shared__ even when the shared instance is running and discovered (issue #142)", async () => {
    // The reserved shared key is intentionally retained across teardown (#51)
    // and is never a worktree. It must survive prune regardless of liveness.
    const tmp = tmpAnchor();
    const liveId = idFor(tmp.worktreeRoot, "main");
    const git = pruneGit({
      anchor: tmp.anchor,
    ...
    const registryRef = {
      snapshot: { [liveId]: 22048, [SHARED_REGISTRY_KEY]: 30000 } as RegistrySnapshot,
    };
    ...
    const result = await runPrune({
      cwd: join(tmp.worktreeRoot, "main"),
      git,
      // The shared instance is running and discovered.
      discover: async () => [instance("shared", { kind: "shared", status: "running" })],
      readRegistry: () => registryRef.snapshot,
      withRegistryLock: async (_anchor, mutate) => {
        const after = await mutate(registryRef.snapshot);
        registryRef.snapshot = after;
